...
## 🏁 Compile and Run
### Disclamir
all commands might not run or change slightly based on your distro. these commands are done with linux mint with cinnamon  
To be able to compile the source C-file, first, we must to open the terminal in the `bin` folder then use this command:
```bash
