...
/* forward and backward branches counters */
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
 ...
 stat_reg_counter(sdb, "forward_taken",
    "number of forward branches that were taken",
   &forward_branches_taken, 0, NULL);
 
  stat_reg_counter(sdb, "forward_not_taken",
    "number of forward branches that were not taken",
    &forward_branches_not_taken, 0, NULL);

  stat_reg_counter(sdb, "backward_taken",
    "number of backward branches that were taken",
    &backward_branches_taken, 0, NULL);

 stat_reg_counter(sdb, "backward_not_taken",
    "number of backward branches that were not taken",
    &backward_branches_not_taken, 0, NULL);
  /*commited forward and backward branches*/ 
   stat_reg_counter(sdb, "committed_forward_taken",
    "number of committed forward branches that were taken",
    &committed_forward_taken, 0, NULL);

  stat_reg_counter(sdb, "committed_forward_not_taken",
    "number of committed forward branches that were not taken",
    &committed_forward_not_taken, 0, NULL);

  stat_reg_counter(sdb, "committed_backward_taken",
    "number of committed backward branches that were taken",
    &committed_backward_taken, 0, NULL);

  stat_reg_counter(sdb, "committed_backward_not_taken",
    "number of committed backward branches that were not taken",
    &committed_backward_not_taken, 0, NULL);
...
#include "machine.def"
	...
	sim_total_branches++;
	  /* Count forward and backward branches */
